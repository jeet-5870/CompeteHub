...
const Signup = () => {
  ...
  const mapAuthError = (err) => {
    ...
    if (err.code === 'auth/email-already-in-use') themedMessage = "This email is already registered. Please sign in instead.";
    if (err.code === 'auth/invalid-email') themedMessage = "The email address is badly formatted.";
  ...
  const handleSignup = async (e) => {
    ...
    await authenticate(firebaseAuth.register(username, email, password));
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden bg-[var(--color-bg-primary)] text-[var(--color-text-primary)]">
      ...
      <div className="w-full max-w-[380px] bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-xl p-8 z-10 shadow-[0_0_0_1px_rgba(255,255,255,0.05)]">
        <h1 className="text-[20px] font-semibold mb-6 flex justify-center">Create your account</h1>
        ...
        <form onSubmit={handleSignup} className="flex flex-col gap-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-4 w-full bg-[var(--color-btn-primary-bg)] border border-[var(--color-btn-primary-border)] text-white text-sm font-medium rounded-md py-1.5 hover:brightness-90 transition-all font-mono tracking-wide disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      <div className="mt-8 text-sm z-10 text-[var(--color-text-secondary)] border border-[var(--color-border)] rounded-xl py-4 px-12 bg-[var(--color-bg-secondary)] shadow-sm text-center">
        Already have an account? {' '}
        <Link to="/login" className="flex-1 text-[var(--color-accent-blue)] hover:underline font-medium">Sign in</Link>
      </div>
    </div>
  );
};
